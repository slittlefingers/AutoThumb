def get_action_prompt(instruction, clickable_infos, width, height, keyboard, summary_history, action_history, last_summary, last_action, add_info, error_flag, completed_content, memory,home_flag):
    prompt = "### Background ###\n"
    ...
    prompt += "Now you need to combine all of the above to perform just one action on the current page. You must choose one of the seven actions below:\n"
    prompt += "Pause video: If the current page is the normal video and instruction let you to pause the video, you have to use this action to pause the video. but if it is shorts please using tap (x,y) instead\n"
    ...
    if home_flag: # home flag show if it is the last step we gonna stop and shut the app
